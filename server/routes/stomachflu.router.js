...
router.get('/', rejectUnauthenticated, (req, res) => {
    const queryText = `
        SELECT 
            ...
            "student"."first_name", 
            "student"."grade", 
            "stomach_flu_record"."illness_date", 
            "illness"."name" AS "illness", 
            "stomach_flu_record"."symptoms"
        FROM 
            "student"
        JOIN 
            "stomach_flu_record" ON "student"."id" = "stomach_flu_record"."student_id"
...
router.post('/', rejectUnauthenticated, (req, res) => {
    ...
    const insertIllnessInputQuery = `
        INSERT INTO "stomach_flu_record" ("illness_id", "symptoms", "illness_date", "student_id")
        VALUES ($1, $2, $3, $4)
    `;
            ...
            return pool.query(insertIllnessQuery, illnessValues)
                .then((illnessResult) => {
                    ...
                    return pool.query(insertIllnessInputQuery, illnessInputValues)
                        ...
                        .catch((error) => {
                            console.error('Error inserting into stomach_flu_record:', error);
                            res.sendStatus(500);
                        });
                })
                .catch((error) => {
...
router.put('/:id', (req, res) => {
    ...
    const updateIllnessQuery = `
        UPDATE "illness"
        SET "name" = $1
        WHERE "id" = (SELECT "illness_id" FROM "stomach_flu_record" WHERE "student_id" = $2)
    `;

    const updateIllnessInputQuery = `
        UPDATE "stomach_flu_record"
